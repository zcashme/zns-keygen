...
mod tests {
    ...
    const TEST_VECTORS: [TestVector; 1] = [TestVector {
        root_seed: [
    ...
    fn roundtrip_parse_and_display() {
        for tv in TEST_VECTORS {
            let fp = SeedFingerprint::from_seed(&tv.root_seed).unwrap();
            let s = fp.to_string();
            let parsed: SeedFingerprint = s.parse().expect("round-trip parse");
            assert_eq!(parsed.to_bytes(), fp.to_bytes());
        }
    }
